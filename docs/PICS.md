...
For the **BACnet B-SS (Smart Sensor) Node.js example** -
...
| **Product Name** | CAS BACnet Stack Example - B-SS (Node.js) |
...
there is no `RegisterCallbackSetProperty*` registration anywhere in this
example, so the stack has nothing to consult for a write and refuses it on its
own.
